...
def parse_ontology_file(file_path: str) -> List[Tuple[Any, str, Any]]:
    ...
    FAIL FAST: Any parsing errors will raise exceptions immediately
    """
    ...
    try:
        ...
        raise RuntimeError(f"❌ Failed to read ontology file {file_path}: {e}")
    ...
    # Process all statements with FAIL FAST
    for line_num, statement in statements:
        try:
            ...
            raise ParseError(f"❌ Line {line_num}: Parse error in '{statement}': {e.message}", 
                             e.position, e.remaining_text)
        except Exception as e:
            raise RuntimeError(f"❌ Line {line_num}: Unexpected error in '{statement}': {e}")
